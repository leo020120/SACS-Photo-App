...
async function noFaceEmail(emailAddress) {
...
const email = {
    ...
    body: {
        contentType: "Text",
        content: `Hello ${emailAddress}, \n \n Your photo has been rejected as there was no face detected in the image. Please resolve this issue before re-submitting \n If you have any further questions please contact id.card@imperial.ac.uk`
    }
};
...
async function sendEmail() {
    ...
    const client = Client.initWithMiddleware({ authProvider: customAuthProvider });

    // Send the email
    try {
        const emailResponse = await client.api('/users/leo.palmer@imperial.ac.uk/sendMail') //REPLACE WITH SERVICE ACCOUNT
            .post({
                message: email,
